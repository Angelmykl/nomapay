...
export default function NomaPay() {
  ...
  const addTx = (tx) => {
    setTxHistory(prev => [{ ...tx, id: Date.now(), time: Date.now(), unread: true }, ...prev].slice(0, 50));
  ...
  const fetchTxHistory = async (address) => {
    try {
      ...
      const sentEvents = await contract.queryFilter(contract.filters.TokenSent(), fromBlock);
      const userTag = localStorage.getItem(`nomapay_user_${address}`) || "";
      const txs = [];

      for (const e of sentEvents.reverse().slice(0, 30)) {
        const block = await provider.getBlock(e.blockNumber);
        const tokenSymbol = e.args.token.toLowerCase() === USDC_ADDRESS.toLowerCase() ? "USDC" : "EURC";
        const amount = (Number(e.args.amount) / 1e6).toFixed(2);
        const fee = (Number(e.args.fee) / 1e6).toFixed(4);
        if (e.args.fromUsername === userTag || e.args.toUsername === userTag) {
          txs.push({
            id: e.transactionHash,
            type: e.args.fromUsername === userTag ? "sent" : "received",
            from: e.args.fromUsername,
            to: e.args.toUsername,
            amount, fee,
            token: tokenSymbol,
  ...
  const fetchBalances = async (address) => {
    try {
      ...
      const [uBal, eBal] = await Promise.all([usdc.balanceOf(address), eurc.balanceOf(address)]);
  ...
  const registerUsername = async () => {
    ...
    try {
      ...
      setStep("app");
      fetchTxHistory(account);
      addTx({ type: "registered", tag: regInput });
  ...
  const sendTokens = async () => {
    ...
    try {
      ...
      addTx({ type: "sent", from: username, to: sendTo, amount: net, fee, token: sendToken });
  ...
  const swapTokens = async () => {
    ...
    try {
      ...
      addTx({ type: "swap", from: swapFrom, to: toToken, amount: swapAmount, net, token: swapFrom });
  ...
  const s = {
    ...
    nomaTag:{ padding:"0 14px", color:C.muted, fontSize:13, borderLeft:`1px solid ${C.border}`, height:48, display:"flex", alignItems:"center" },
    ...
    bellBtn:{ background:"rgba(0,229,160,0.08)", border:`1px solid ${C.border}`, borderRadius:20, width:36, height:36, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:16, position:"relative" },
    badge:{ position:"absolute", top:-4, right:-4, background:C.error, color:"#fff", borderRadius:"50%", width:18, height:18, fontSize:10, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center" },
    notifPanel:{ position:"absolute", top:44, right:0, width:320, background:C.card, border:`1px solid ${C.border}`, borderRadius:14, boxShadow:"0 8px 40px rgba(0,0,0,0.4)", zIndex:200, overflow:"hidden" },
    notifHeader:{ padding:"14px 16px", borderBottom:`1px solid ${C.border}`, display:"flex", justifyContent:"space-between", alignItems:"center" },
    ...
    notifItem:{ padding:"12px 16px", borderBottom:`1px solid ${C.border}`, display:"flex", gap:10, alignItems:"flex-start" },
    notifIcon:{ width:32, height:32, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, flexShrink:0 },
    notifBody:{ flex:1 },
    notifMain:{ fontSize:12, fontWeight:600, marginBottom:2 },
    ...
    notifHash:{ fontSize:10, color:C.accent, textDecoration:"underline", marginTop:2, display:"block" },
  };
              ...
              {showNotif && (
                <div style={s.notifPanel}>
                  <div style={s.notifHeader}>
                    ...
                    <button style={s.notifClear} onClick={() => setTxHistory([])}>Clear all</button>
                  </div>
                  <div style={s.notifList}>
                    {txHistory.length === 0 ? (
                      ...
                      return (
                        <div key={tx.id} style={{...s.notifItem, background:tx.unread?"rgba(0,229,160,0.03)":"transparent"}}>
                          ...
                          <div style={s.notifBody}>
                            ...
                            {tx.hash && (
                              <a href={`https://testnet.arcscan.app/tx/${tx.hash}`} target="_blank" rel="noreferrer" style={s.notifHash}>
                                View on explorer ↗
                              </a>
                            )}
                          </div>
      ...
      {step==="register" && (
        <div style={s.center}>
          <div style={s.card}>
            ...
            {regInput && <div style={{fontSize:12, color:C.accent, marginBottom:12, textAlign:"center"}}>Your tag: <strong>{regInput}.noma</strong></div>}
        ...
        <div style={s.appWrap}>
          ...
          {tab==="send" && (
            <div style={s.panel}>
              ...
              <button style={{...s.btnPrimary,marginTop:20,opacity:sendStatus==="pending"?0.6:1}} onClick={sendTokens} disabled={sendStatus==="pending"}>
          ...
          {tab==="swap" && (
            <div style={s.panel}>
              ...
              <button style={{...s.btnPrimary,marginTop:20,opacity:swapStatus==="pending"?0.6:1}} onClick={swapTokens} disabled={swapStatus==="pending"}>
